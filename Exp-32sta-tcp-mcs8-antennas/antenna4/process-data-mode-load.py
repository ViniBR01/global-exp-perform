...
normalized_load = avg_bits_sec / (86700000)
...
ax.errorbar(normalized_load[0][0:i], average_dl[0][0:i], yerr=error_bar_dl[0][0:i], color='tab:blue')
ax.errorbar(normalized_load[1][0:i], average_dl[1][0:i], yerr=error_bar_dl[1][0:i], color='tab:orange')
ax.errorbar(normalized_load[2][0:i], average_dl[2][0:i], yerr=error_bar_dl[2][0:i], color='tab:green')
# Plot Uploads as a dashed line
ax.errorbar(normalized_load[0][0:i], average_ul[0][0:i], yerr=error_bar_ul[0][0:i], linestyle='dashed', color='tab:blue')
ax.errorbar(normalized_load[1][0:i], average_ul[1][0:i], yerr=error_bar_ul[1][0:i], linestyle='dashed', color='tab:orange')
ax.errorbar(normalized_load[2][0:i], average_ul[2][0:i], yerr=error_bar_ul[2][0:i], linestyle='dashed', color='tab:green')
...
# # Plot Downloads as a solid line
# ax2.plot(x[0:i], (average_dl[1][0:i] / average_dl[0][0:i] - 1), '*-', color='tab:orange')
# ax2.plot(x[0:i], (average_dl[2][0:i] / average_dl[0][0:i] - 1), '*-', color='tab:green')
# # Plot Uploads as a dashed line
# ax2.plot(x[0:i], (average_ul[1][0:i] / average_ul[0][0:i] - 1), '*--', color='tab:orange')
# ax2.plot(x[0:i], (average_ul[2][0:i] / average_ul[0][0:i] - 1), '*--', color='tab:green')
...
i = 7
# Plot Downloads as a solid line
ax3.errorbar(normalized_load[0][h:i], (average_dl[0][h:i] + average_ul[0][h:i])/2, yerr=(error_bar_dl[0][h:i] + error_bar_ul[0][h:i])/2, color='tab:blue')
ax3.errorbar(normalized_load[1][h:i], (average_dl[1][h:i] + average_ul[1][h:i])/2, yerr=(error_bar_dl[1][h:i] + error_bar_ul[1][h:i])/2, color='tab:orange')
ax3.errorbar(normalized_load[2][h:i], (average_dl[2][h:i] + average_ul[2][h:i])/2, yerr=(error_bar_dl[2][h:i] + error_bar_ul[2][h:i])/2, color='tab:green')
# # Plot Uploads as a dashed line
# ax3.errorbar(normalized_load[0][0:i], average_ul[0][0:i], yerr=error_bar_ul[0][0:i], linestyle='dashed', color='tab:blue')
# ax3.errorbar(normalized_load[1][0:i], average_ul[1][0:i], yerr=error_bar_ul[1][0:i], linestyle='dashed', color='tab:orange')
# ax3.errorbar(normalized_load[2][0:i], average_ul[2][0:i], yerr=error_bar_ul[2][0:i], linestyle='dashed', color='tab:green')

ax3.legend(['Single-user uplink','MU uplink - Reports-based','MU uplink - perfect backlog'], loc='upper right') #loc='lower left')
# ax.legend(bbox_to_anchor=(1.1, 1.05))
ax3.set(xlim=(0, 2), ylim=(0, 60.0))
ax3.grid(color='k', linestyle='--', linewidth=1)
plt.title("Average TCP throughput of a 300kB file - 32 stations - 4x4 MIMO")
plt.xlabel('Aggregate traffic load in network [Normalized by single-stream PHY rate]') 
plt.ylabel('Measured average TCP Throughput per file, in Mbps') 
plt.savefig('TCP-Throughput-averages-32sta.png', dpi=300)

print('su uplink: load, mbps, error')
print(normalized_load[0])
print((average_dl[0] + average_ul[0])/2)
print((error_bar_dl[0] + error_bar_ul[0])/2)

print('mu uplink: load, mbps, error')
print(normalized_load[2])
print((average_dl[2] + average_ul[2])/2)
print((error_bar_dl[2] + error_bar_ul[2])/2)
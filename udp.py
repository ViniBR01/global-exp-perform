...
def run_iperf_DL(server_ip, filesize):
    return subprocess.check_output(["iperf3", "-c", server_ip, "-u", "-b 80M", "-J", "-M 1448", "-l 1448", "-n "+str(filesize)], timeout=15)

def run_iperf_UL(server_ip, filesize):
    return subprocess.check_output(["iperf3", "-c", server_ip, "-u", "-b 80M", "-J", "-M 1448", "-l 1448", "-n "+str(filesize), "-R"], timeout=15)
...
def extract_all_data(json_out):
    ...
    data_obj = JSON_Data()
    # Get receiver info:
    data_obj.bps_receiver = dict_out["end"]["streams"][0]["udp"]["bits_per_second"]
    data_obj.bytes_receiver = dict_out["end"]["streams"][0]["udp"]["bytes"]
    data_obj.seconds_receiver = dict_out["end"]["streams"][0]["udp"]["seconds"]
    # Get sender info:
    data_obj.bps_sender = dict_out["end"]["streams"][0]["udp"]["bits_per_second"]
    data_obj.bytes_sender = dict_out["end"]["streams"][0]["udp"]["bytes"]
    data_obj.seconds_sender = dict_out["end"]["streams"][0]["udp"]["seconds"]

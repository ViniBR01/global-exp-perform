...
def run_iperf_DL(server_ip, filesize):
    return subprocess.check_output(["iperf3", "-c", server_ip, "-u", "-b 60M", "-J", "-M 1400", "-l 1400", "-n "+str(filesize)])

def run_iperf_UL(server_ip, filesize):
    return subprocess.check_output(["iperf3", "-c", server_ip, "-u", "-b 60M", "-J", "-M 1400", "-l 1400", "-n "+str(filesize), "-R"])
...
def extract_bps(json_out):
    ...
    return dict_out["end"]["streams"][0]["udp"]["bits_per_second"]
